error id: file://<WORKSPACE>/exercise2/process-simulator/server/src/main/scala/cp/serverSim/Routes.scala:info.
...
non-local guesses:
	 -org/http4s/logger/info.
	 -org/http4s/logger/info#
	 -org/http4s/logger/info().
	 -org/http4s/dsl/io/logger/info.
	 -org/http4s/dsl/io/logger/info#
	 -org/http4s/dsl/io/logger/info().
	 -java/util/concurrent/atomic/logger/info.
	 -java/util/concurrent/atomic/logger/info#
	 -java/util/concurrent/atomic/logger/info().
	 -logger/info.
	 -logger/info#
...
object Routes {
  ...
  */
  ...
  private def runProcess(cmd: String, userIp: String): String = {
    ...
    logger.inf@@o(s"🔹 Starting processes (${cnt}) for user $userIp:" +
      s"${cmds.map("\n - "+_).mkString}")
