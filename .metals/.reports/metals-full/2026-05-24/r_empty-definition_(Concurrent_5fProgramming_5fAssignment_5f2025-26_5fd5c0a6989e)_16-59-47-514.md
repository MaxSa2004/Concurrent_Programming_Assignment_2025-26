error id: file://<WORKSPACE>/exercise2/process-simulator/server/src/main/scala/cp/serverSim/Routes.scala:decrementAndGet
file://<WORKSPACE>/exercise2/process-simulator/server/src/main/scala/cp/serverSim/Routes.scala
empty definition using pc, found symbol in pc: 
semanticdb not found

found definition using fallback; symbol decrementAndGet
offset: 5866
...
object Routes {
  ...
  // ex2.4 using @volatile var
  @volatile private var firstFinished: Boolean = false
    ...
    */
    ...
    def executeInstruction(i: Int): Unit = {
      ...
      thread_pool.execute {
        ...
        // ex2.4 volatile vars
        synchronized(if (!firstFinished){
            println(s"[${timestamp}] FIRST FINISHED:  i=${i+1} (${instruct.instruction})")
            firstFinished = true;
          })
        ...
        if(done(i).compareAndSet(false, true)){
          dependencies(i).foreach{j =>
            val left = remaining(j).decrementAnd@@Get()
            if (left == 0){
