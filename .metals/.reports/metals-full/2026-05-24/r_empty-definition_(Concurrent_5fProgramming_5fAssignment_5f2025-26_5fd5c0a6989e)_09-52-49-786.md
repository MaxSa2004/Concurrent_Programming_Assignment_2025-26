error id: file://<WORKSPACE>/exercise2/process-simulator/server/src/main/scala/cp/serverSim/Routes.scala:split.
file://<WORKSPACE>/exercise2/process-simulator/server/src/main/scala/cp/serverSim/Routes.scala
empty definition using pc, found symbol in pc: split.
semanticdb not found
empty definition using fallback
non-local guesses:
	 -org/http4s/noComment/split.
	 -org/http4s/noComment/split#
	 -org/http4s/noComment/split().
	 -org/http4s/dsl/io/noComment/split.
	 -org/http4s/dsl/io/noComment/split#
	 -org/http4s/dsl/io/noComment/split().
	 -noComment/split.
	 -noComment/split#
	 -noComment/split().
	 -scala/Predef.noComment.split.
	 -scala/Predef.noComment.split#
	 -scala/Predef.noComment.split().
offset: 4049
...
object Routes {
  ...
  */
  ...
  private def parseIgnoreAfter(rawCommand: String): (String, Int) = {
    ...
    val delay = if(parts.length == 2 && parts(1).nonEmpty) parts(1).toInt else 0
