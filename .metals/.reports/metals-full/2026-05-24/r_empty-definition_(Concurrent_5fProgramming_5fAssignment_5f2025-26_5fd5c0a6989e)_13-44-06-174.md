error id: file://<WORKSPACE>/exercise2/process-simulator/server/src/main/scala/cp/serverSim/Routes.scala:indices.
...
non-local guesses:
	 -org/http4s/parsedInstr/indices.
	 -org/http4s/dsl/io/parsedInstr/indices.
	 -java/util/concurrent/atomic/parsedInstr/indices.
	 -parsedInstr/indices.
	 -scala/Predef.parsedInstr.indices.
offset: 5711
  ...
  */
  ...
  private def runProcess(cmd: String, userIp: String): String = {
    ...
    */
    ...
    // start all ready instructions (where no dependencies)
    parsedInstr.in@@dices.foreach{ i =>
