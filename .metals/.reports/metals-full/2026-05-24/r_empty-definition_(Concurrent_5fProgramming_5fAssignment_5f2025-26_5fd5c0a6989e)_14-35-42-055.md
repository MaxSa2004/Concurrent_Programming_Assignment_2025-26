...
non-local guesses:
	 ...
	 -org/http4s/s/split#
	 -org/http4s/s/split().
	 -org/http4s/dsl/io/s/split.
	 -org/http4s/dsl/io/s/split#
	 -org/http4s/dsl/io/s/split().
	 -java/util/concurrent/atomic/s/split.
	 -java/util/concurrent/atomic/s/split#
	 -java/util/concurrent/atomic/s/split().
	 -s/split.
	 -s/split#
	 -s/split().
	 -scala/Predef.s.split.
	 -scala/Predef.s.split#
	 -scala/Predef.s.split().
offset: 7422
  ...
  */
  ...
  private def parseAll(rawCommand: String): Instr = {
    ...
    val afterDependencies: List[Int] = afterPart match {
      case None => Nil
      case Some(s) => if (s.isEmpty) Nil else s.s@@plit(",")
          .toList
          .map(_.trim)
          .filter(_.nonEmpty)
          .flatMap(_.toIntOption)  // avoids crashing
          .map(_ - 1)
          .distinct
    }
