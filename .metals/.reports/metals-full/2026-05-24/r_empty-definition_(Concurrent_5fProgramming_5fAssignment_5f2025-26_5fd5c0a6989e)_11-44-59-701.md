error id: file://<WORKSPACE>/exercise2/process-simulator/server/src/main/scala/cp/serverSim/Routes.scala:length.
...
non-local guesses:
	 -org/http4s/parts/length.
	 -org/http4s/parts/length#
	 -org/http4s/parts/length().
	 -org/http4s/dsl/io/parts/length.
	 -org/http4s/dsl/io/parts/length#
	 -org/http4s/dsl/io/parts/length().
	 -parts/length.
	 -parts/length#
	 -parts/length().
	 -scala/Predef.parts.length.
	 -scala/Predef.parts.length#
	 -scala/Predef.parts.length().
offset: 5272
  ...
  */
  ...
  private def parseAll(rawCommand: String): (String, Int, List[Int]) = {
    ...
    val afterDependencies: List[Int] = afterPart match {
      ...
      case Some(s) => s.split(",").toList.map(_.trim).map(_.toInt).map(_ - 1)
    }

    val atSplit = leftSide.split("@", 2)
    val instruction = atSplit(0).trim
    // if no @ then it is 0 delay by default
    val delay = if (atSplit.@@length == 2 && parts(1).nonEmpty) parts(1).trim.toInt  else 0

    // converted to seconds
    (instruction, delay * 1000, afterDependencies)
  }
